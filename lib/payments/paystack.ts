  ...
  try {
    ...
    if (!customerData.status && !customerData.data?.customer_code) {
      ...
      const existingCustomer = await fetch(
        `${paystackConfig.baseUrl}/customer/${encodeURIComponent(customer.email)}`,
        {
          headers: {
            'Authorization': `Bearer ${paystackConfig.secretKey}`,
          },
...
export async function cancelPaystackSubscription(
  ...
  try {
    const response = await fetch(`${paystackConfig.baseUrl}/subscription/disable`, {
      method: 'POST',
      headers: {
        'Authorization': `Bearer ${paystackConfig.secretKey}`,
        'Content-Type': 'application/json',
...
export function getPaystackCurrencySymbol(countryCode: PaystackCountry): string {
