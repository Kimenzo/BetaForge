...
import { PaymentProvider, PAYSTACK_COUNTRIES, PaystackCountry, PAYSTACK_CURRENCIES } from './types';
...
export function detectPaymentProvider(countryCode: string): PaymentProvider {
  ...
  if (PAYSTACK_COUNTRIES.includes(upperCode as typeof PAYSTACK_COUNTRIES[number])) {
    return 'paystack';
  }
