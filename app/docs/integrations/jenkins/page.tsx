...
export default function JenkinsIntegrationPage() {
  const [activeExample, setActiveExample] = useState<
    "declarative" | "scripted" | "sharedLibrary" | "blueOcean"
  >("declarative");
          ...
          <div className="flex items-start gap-6 mb-12">
            ...
            <div className="flex-1">
              ...
              <p className="text-lg text-phantom-gray mb-6">
                Integrate BetaForge AI testing into your Jenkins pipelines with
                declarative or scripted syntax. Perfect for enterprise CI/CD
                workflows.
              </p>
              <div className="flex items-center gap-4">
                <Link
            ...
            <div className="p-6 bg-void-elevated border border-white/5 rounded-xl">
              ...
              <ol className="space-y-3">
                ...
                <li className="flex items-start gap-3">
                  ...
                  <span className="text-sm text-phantom-gray">
                    Enter your BetaForge API key as the secret
                  </span>
                </li>
                <li className="flex items-start gap-3">
                  <span className="w-6 h-6 rounded-full bg-neural/20 text-neural-bright flex items-center justify-center text-xs font-bold flex-shrink-0">
          ...
          <div className="mt-6 p-4 bg-void-elevated border border-white/5 rounded-xl">
            {activeExample === "declarative" && (
              <div>
                ...
                <p className="text-sm text-phantom-gray">
                  The recommended approach for most use cases. Uses a clean,
                  structured syntax with stages and steps. This example includes
                  deployment stages, BetaForge testing with failure detection,
            ...
            {activeExample === "scripted" && (
              <div>
                ...
                <p className="text-sm text-phantom-gray">
                  Full Groovy flexibility for complex workflows. Uses the
                  BetaForge CLI for cleaner integration and archives test
            ...
            {activeExample === "sharedLibrary" && (
              <div>
                ...
                <p className="text-sm text-phantom-gray">
                  Create a reusable{" "}
                  <code className="text-electric-cyan">betaforgeTest</code> step
                  that can be used across all your Jenkins pipelines. Perfect
                  for enforcing consistent testing practices across teams.
                </p>
              </div>
            )}
            {activeExample === "blueOcean" && (
              <div>
                ...
                <p className="text-sm text-phantom-gray">
                  Optimized for Blue Ocean visualization with parallel stages,
                  manual approval gates, and polling-based status checks for
                  long-running tests.
                </p>
              </div>
            )}
                ...
                {[
                  {
                    ...
                    desc: "Your BetaForge API key (use credentials binding)",
                    required: true,
                  },
                  {
                    name: "BETAFORGE_PROJECT_ID",
                    desc: "The project ID to test against",
                    required: true,
                  },
                  {
                    name: "BETAFORGE_TEST_URL",
                    desc: "The URL to test (defaults to staging URL)",
                    required: false,
                  },
                  {
                    name: "BETAFORGE_AGENTS",
                    desc: "Comma-separated list of agent names",
                    required: false,
                  },
                  {
                    name: "BETAFORGE_TIMEOUT",
                    desc: "Maximum wait time in seconds (default: 600)",
                    required: false,
                  },
                  {
                    name: "BETAFORGE_FAIL_ON_CRITICAL",
                    desc: "Fail build on critical bugs (default: true)",
                    required: false,
                  },
                ].map((env, i, arr) => (
                  <tr
                  ...
                  >
                    <td className="px-4 py-3">
                      <code className="text-xs bg-neural/20 text-neural-bright px-2 py-1 rounded font-mono">
                        {env.name}
                      </code>
                    </td>
                    <td className="px-4 py-3">
            ...
            {[
              {
                ...
                solution:
                  "Ensure the credential ID matches exactly. Check that the credential scope includes your pipeline's folder. Use `withCredentials` block for proper binding.",
              },
              {
                problem: "curl command fails",
                solution:
                  "Make sure `curl` and `jq` are installed on your Jenkins agents. Consider using the HTTP Request plugin instead for better error handling.",
              },
              {
                problem: "Pipeline times out",
                solution:
                  "BetaForge tests can take 5-15 minutes. Increase your stage timeout or use async mode with polling to avoid blocking the executor.",
              },
              {
                problem: "JSON parsing errors",
                solution:
                  "Install the Pipeline Utility Steps plugin for `readJSON`. Alternatively, use `jq` in shell commands to extract values.",
              },
              {
                problem: "Tests not starting",
                solution:
                  "Verify your staging environment is accessible from Jenkins agents. Check firewall rules and DNS resolution.",
              },
            ].map((item) => (
              <div
                key={item.problem}
