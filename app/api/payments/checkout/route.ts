...
export async function POST(request: NextRequest) {
  try {
      ...
      return NextResponse.json(
        {
          ...
          contactUrl: "/contact?type=enterprise",
        },
        { status: 400 }
      );
    }
    ...
    const detectedCountry =
      country || detectCountryFromRequest(request.headers) || "US";
    let provider = detectPaymentProvider(detectedCountry);

    // Debug logging
    if (process.env.NODE_ENV === "development") {
      console.log("[Checkout] Provider detection:", {
        detectedCountry,
        provider,
      });
    }
    ...
    if (provider === "paystack" && !config.paystack) {
      if (config.dodo) {
        console.log(
          "[Checkout] Paystack not configured, falling back to Dodo Payments"
        );
        provider = "dodo";
      } else {
        return NextResponse.json(
          {
            error:
              "Payment provider not configured. Please contact support or add PAYSTACK_SECRET_KEY and PAYSTACK_PUBLIC_KEY to your environment.",
          },
          { status: 503 }
        );
      }
    ...
    if (provider === "dodo" && !config.dodo) {
      return NextResponse.json(
        {
          error:
            "Dodo Payments not configured. Please add DODO_PAYMENTS_API_KEY to your environment.",
        },
        { status: 503 }
      );
    }
...
export async function GET(request: NextRequest) {
  ...
  const country =
    ...
    detectCountryFromRequest(request.headers) ||
    "US";
